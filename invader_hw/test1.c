...
int main()
{
	...
	map_init(&gPotalModel);
	map_load(&gPotalModel,"potal.dat");

	Plane_init(&gPlayerObject,&gPlayerModel,30,26);
	Potal_init(&gPotalObject,&gPotalModel,2,2);
	...
	while(bLoop) {
		...
		if(acc_tick > 0.1) {
			...
			gPotalObject.pfDraw(&gPotalObject,&gScreenBuf[1]);
